...
void main() {
    ...
    vec2 aspect2 = u_resolution.x > u_resolution.y
        ? vec2(u_resolution.x / u_resolution.y, 1.0)
        : vec2(1.0, u_resolution.y / u_resolution.x);

    float sdf;
    if (VAR == 0) {
        sdf = sdRoundRect(st, vec2(size) * aspect2, roundness);
...
const ShapeBlur = ({
  ...
  }, [variation, pixelRatioProp, shapeSize, roundness, borderSize, circleSize, circleEdge, color[0], color[1], color[2]]);
